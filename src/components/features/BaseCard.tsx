      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex-1">
          <div className="flex items-center gap-2 sm:gap-3 mb-2 sm:mb-3">
            <div className={`flex h-10 w-10 sm:h-12 sm:w-12 items-center justify-center rounded-xl bg-gradient-to-br ${colors.gradient} text-white shadow-soft group-hover:scale-110 transition-transform duration-300`}>
              <span className="text-base font-bold sm:text-lg">{title.charAt(0).toUpperCase()}</span>
            </div>
            <div className="flex-1">
              <h3 className={`text-lg font-bold text-gray-900 dark:text-white ${colors.hover} transition-colors sm:text-xl`}>
                {title}
              </h3>
              {(pronunciationVariants?.length || pronunciationIpa) && (
                <div className="mt-1">
                  <PronunciationPlayer
                    variants={pronunciationVariants || (pronunciationIpa ? [{ accent: settings.fallbackAccent as AccentType, ipa: pronunciationIpa }] : [])}
                    className="text-sm"
                  />
                </div>
              )}
          ...
          <p className="text-sm text-gray-600 dark:text-gray-300 leading-relaxed mb-3 sm:mb-4 sm:text-base">{meaning}</p>

          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2 sm:gap-4">
              <span className={`inline-flex items-center rounded-full ${colors.badge} px-3 py-1 text-xs font-semibold`}>
                {badge}
              </span>
              {subtitle && (
        ...
        <div className="opacity-0 group-hover:opacity-100 transition-opacity duration-300">
          <svg className={`h-5 w-5 ${colors.arrow}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
