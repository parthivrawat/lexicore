...
export function StatisticsDashboard() {
  ...
  const stats = useMemo(() => {
    const prefixCount = roots.filter(r => r.type === 'prefix').length;
    const suffixCount = roots.filter(r => r.type === 'suffix').length;
    const baseCount = roots.filter(r => r.type === 'base').length;
  ...
  const StatCard = ({
    ...
    <div className="card-hover bg-white dark:bg-gray-800 rounded-2xl border border-gray-200/50 dark:border-gray-700/50 p-6 shadow-soft">
      <div className="flex items-center gap-3">
        <div
          className={`flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br ${color} text-white`}
        >
  ...
  return (
    <div
      className="grid gap-4 sm:gap-6 grid-cols-2 lg:grid-cols-3 animate-slide-up"
